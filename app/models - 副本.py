...
class User(UserMixin,db.Model):
	...
	#用户位置表
	user_positions = db.relationship('Position', backref='user_position')

	#用户优惠券
	user_coupons = db.relationship('Coupons', backref='user_coupons')
...
class Consignor(db.Model):
	...
	consignor_images = db.relationship('Consignor_images', backref='consignorImages',lazy='dynamic')
...
class Goods(db.Model):
	...
	#状态 -2失效订单超时未支付  -1管理员关闭 0发布 1司机已经接单未付款 ，2司机已付款到系统  3司机已经运送抵达ok  4系统或货主确认已经抵达ok 完成  5货主确认了司机的接单信息  6货主已经在线支付运费 。其他状态等待 ?4初期未付款状态?
	state = db.Column(db.Integer(),default=0)
