...
from app import db,scheduler,flask_wechat
...
def add_posts():
	...
	try:
		...
		flash(u'车运信息发布成功，系统自动进行信息匹配，匹配成功后将会通知您，请稍后。','success')
		#应该添加定时操作
...
def show_post_p():
	...
	if not chechang or not chexing or not number or not request.files['xingshizheng'] or not request.files['jiashizheng'] or not request.files['chetouzhao'] or not request.files['cheweizhao']:
		flash(u'信息录入不正确，请重新输入','error')
	...
	try:
		if int(chechang)!=0:
			...
			#删除原来图片
			driver_images = Driver_images.query.filter_by(driver=drivers).all()
			for i in driver_images:
				db.session.delete(i)
			db.session.commit()

			xingshizheng = request.files['xingshizheng']
			fname, fext = os.path.splitext(xingshizheng.filename)
			rnd_name = '%s%s' % (gen_rnd_filename(), fext)
			filepath = os.path.join(current_app.static_folder, 'uploads/driver', rnd_name)
			dirname = os.path.dirname(filepath)
			if not os.path.exists(dirname):
				os.makedirs(dirname)
			xingshizheng.save(filepath)	
			saveurl = '/static/uploads/driver/'+rnd_name			
			db.session.add(Driver_images(name=u'行驶证',url=saveurl,driver=drivers))

			jiashizheng = request.files['jiashizheng']
			fname, fext = os.path.splitext(jiashizheng.filename)
			rnd_name = '%s%s' % (gen_rnd_filename(), fext)
			filepath = os.path.join(current_app.static_folder, 'uploads/driver', rnd_name)
			dirname = os.path.dirname(filepath)
			if not os.path.exists(dirname):
				os.makedirs(dirname)
			jiashizheng.save(filepath)	
			saveurl = '/static/uploads/driver/'+rnd_name			
			db.session.add(Driver_images(name=u'驾驶证',url=saveurl,driver=drivers))

			chetouzhao = request.files['chetouzhao']
			fname, fext = os.path.splitext(chetouzhao.filename)
			rnd_name = '%s%s' % (gen_rnd_filename(), fext)
			filepath = os.path.join(current_app.static_folder, 'uploads/driver', rnd_name)
			dirname = os.path.dirname(filepath)
			if not os.path.exists(dirname):
				os.makedirs(dirname)
			chetouzhao.save(filepath)
			saveurl = '/static/uploads/driver/'+rnd_name	
			db.session.add(Driver_images(name=u'车头照',url=saveurl,driver=drivers))

			cheweizhao = request.files['cheweizhao']
			fname, fext = os.path.splitext(cheweizhao.filename)
			rnd_name = '%s%s' % (gen_rnd_filename(), fext)
			filepath = os.path.join(current_app.static_folder, 'uploads/driver', rnd_name)
			dirname = os.path.dirname(filepath)
			if not os.path.exists(dirname):
				os.makedirs(dirname)
			cheweizhao.save(filepath)
			saveurl = '/static/uploads/driver/'+rnd_name	
			db.session.add(Driver_images(name=u'车尾照',url=saveurl,driver=drivers))
		...
		try:
			flask_wechat.message.send_text(current_user.wx_open_id,\
				u'您已经提交了车辆信息的认证，请等待客服的审核。')
		except Exception, e:
			print str(e)
		
		flash(u'您已经提交了车辆信息的认证，请等待客服的审核。','success')
...
def send_order():
	...
	if current_user.phone=='' or current_user.phone ==None:
		...
		cs = Consignor.query.filter_by(user=current_user).first()
	
	selfgoodspost = Goods_self_order.query.filter(Goods_self_order.driver_post==dp).filter(Goods_self_order.consignor==cs).first()
	if selfgoodspost:
		flash(u'您已经预约过了，请不要重复预约','error')
		return redirect(url_for('main.index'))
	else:
		address = selfgoodspost.goodsed.start_shi+"——"+selfgoodspost.goodsed.end_shi
		flask_wechat.message.send_text(current_user.wx_open_id,\
			u'您预约了[%s]的车辆信息，请等待司机的确认。'%address)
		
		flash(u'预约成功，系统审核通过将电话通知您。','success')
		db.session.add(Goods_self_order(driver_post=dp,consignor=cs,price=request.form.get('price')))
